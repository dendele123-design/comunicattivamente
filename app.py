...
st.markdown(f"""
    ...
    .timer-box {{
        ...
        padding: 20px; border: 4px dashed {ROSSO_BRAND}; border-radius: 20px; margin: 20px 0;
...
domande_ansia = [
    ...
    {"area": "TEMPO", "testo": "LE TUE RIUNIONI HANNO UN ORDINE SCRITTO?", "sotto": "Tutti sanno di cosa si parla o lo scoprite lì?", "lezione": "Una riunione senza agenda è una chiacchierata al bar costosa. Senza obiettivo, avete solo bruciato stipendi.", "opzioni": [{"t": "🔴 NO, PARLIAMO E BASTA", "p": 1}, {"t": "🟢 SÌ, SEMPRE", "p": 0}]},
    {"area": "TEMPO", "testo": "SEI TU A DECIDERE QUANDO LEGGERE LE MAIL?", "sotto": "O è il 'Ding' del telefono a deciderlo?", "lezione": "La reattività immediata è nevrosi. Il cervello impiega 15 min per ritrovare il focus. Se rispondi subito, reagisci, non lavori.", "opzioni": [{"t": "🔴 APPENA ARRIVANO", "p": 1}, {"t": "🟢 A BLOCCHI ORARI FISSI", "p": 0}]},
    ...
    {"area": "SQUADRA", "testo": "TEST AUTOBUS: SE SPARISCI UN MESE...?", "sotto": "L'azienda produce utile senza di te?", "lezione": "Se l'azienda sei tu, non hai un'azienda. Hai un lavoro a vita. L'obiettivo è rendersi inutili operativamente.", "opzioni": [{"t": "🔴 SI FERMA / CROLLA", "p": 1}, {"t": "🟢 VA AVANTI", "p": 0}]},
    ...
    {"area": "SQUADRA", "testo": "I DIPENDENTI SANNO L'OBIETTIVO DEL MESE?", "sotto": "O vengono solo a timbrare il cartellino?", "lezione": "Non puoi vincere se la squadra non sa il punteggio. Condividere gli obiettivi crea alleati, non mercenari.", "opzioni": [{"t": "🔴 NON CREDO", "p": 1}, {"t": "🟢 SÌ, CONDIVIDIAMO I NUMERI", "p": 0}]},
    ...
    {"area": "STRATEGIA", "testo": "HAI UN PIANO SCRITTO PER I PROSSIMI 12 MESI?", "sotto": "Non un sogno, un piano con date e nomi.", "lezione": "Se è nella testa, è un'allucinazione. Se è scritto, è un progetto. Le aziende si costruiscono sui progetti.", "opzioni": [{"t": "🔴 È NELLA MIA TESTA", "p": 1}, {"t": "🟢 SÌ, SCRITTO E CONDIVISO", "p": 0}]}
]
...
# --- 📊 DIAGNOSI (ANSIA SPA) ---
    ...
    if not st.session_state.ansia_complete:
        ...
        if step < len(domande_ansia):
            ...
            st.session_state.ansia_complete = True; st.rerun()
    else:
        st.subheader("📊 IL RISULTATO DELLA TUA DIAGNOSI")
        score = st.session_state.score_ansia
        if score <= 4:
            titolo, colore, desc = "PROFILO A: L'OROLOGIO SVIZZERO", "#d4edda", "Sei nell'1% degli imprenditori. Hai un sistema, non un lavoro."
        elif score <= 12:
            titolo, colore, desc = "PROFILO B: IL CRICETO STANCO", "#fff3cd", "Sei nella media. L'azienda sta in piedi ma tu sei esausto."
        else:
            titolo, colore, desc = "PROFILO C: L'AZIENDA POSSEDUTA", "#f8d7da", "Allarme Rosso. Sei passeggero di un treno in fiamme."
        ...
        for area, punti in critiche[:3]:
            if punti > 0:
                ...
                if area == "SQUADRA": st.info("👥 **SQUADRA:** Registra un video (Loom) di un compito. Ecco la tua prima procedura.")
                if area == "STRATEGIA": st.info("🎯 **STRATEGIA:** Analizza i clienti. Trova il più tossico e licenzialo.")
...
# --- 🛠️ PRONTO INTERVENTO (TOOLKIT) ---
    ...
    tool = st.radio("Seleziona strumento:", ["💸 Timer dello Spreco", "📑 Generatore SOP", "🧭 Cruscotto Ammiraglio"], horizontal=True)
    st.divider()

    if tool == "💸 Timer dello Spreco":
        st.write("### Quanto ti costa questa riunione?")
        c1, c2 = st.columns(2)
        n_p = c1.number_input("Partecipanti", 1, 50, 4)
        costo_h = c2.number_input("Costo orario medio (€)", 1, 500, 45)
        costo_al_sec = (n_p * costo_h) / 3600

        # LOGICA FIXATA DEL TIMER
        if not st.session_state.timer_running:
            ...
            if st.button("🛑 STOP / RESET RIUNIONE"):
        ...
        if st.button("GENERA SCHEMA", type="primary"): st.code(f"PROCEDURA: {titolo}\n\nSTEP:\n{passaggi}")

    elif tool == "🧭 Cruscotto Ammiraglio":
        st.markdown("<div class='info-box'>Guarda avanti di 30 giorni.</div>", unsafe_allow_html=True)
...
# --- 📖 BIGNAMI ---
    ...
    with st.expander("🛡️ PROTOCOLLO 1: LA FONTE DELLA VERITÀ"): st.write("Addio allegati. Una sola cartella Cloud AZIENDA. Se mandi un file via mail, crei caos.")
